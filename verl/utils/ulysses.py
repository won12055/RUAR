...
def ulysses_pad_and_slice_inputs(input_ids_rmpad: torch.Tensor,
    ...
    and both must be sliced consistently along the sequence axis when
    using Ulysses SP. Otherwise RoPE position_ids stay at full length
    while hidden states are sharded, which breaks attention.
    ...
    if position_ids_rmpad is not None:
        assert position_ids_rmpad.size(-1) == input_ids_rmpad.size(1)
    if sp_size <= 1:
    ...
    if pad_size > 0:
        ...
        if position_ids_rmpad is not None:
            last_pos = position_ids_rmpad[..., -1:]
            pad_offsets = torch.arange(1, pad_size + 1, device=position_ids_rmpad.device, dtype=position_ids_rmpad.dtype)
            pad_shape = (1,) * (position_ids_rmpad.dim() - 1) + (pad_size,)
            pad_offsets = pad_offsets.view(*pad_shape)
            pad_pos_ids = last_pos + pad_offsets
