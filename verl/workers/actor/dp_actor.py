...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        if self.use_remove_padding and unpad_input is None:
            raise ImportError("flash_attn is required when actor_rollout_ref.model.use_remove_padding=True")
        self.ulysses_sequence_parallel_size = self.config.ulysses_sequence_parallel_size
        self.use_ulysses_sp = self.ulysses_sequence_parallel_size > 1

        if os.environ.get('VERL_DISABLE_TORCH_COMPILE') == '1':
            self.compute_entropy_from_logits = verl_F.entropy_from_logits
        else:
            self.compute_entropy_from_logits = torch.compile(verl_F.entropy_from_logits, dynamic=True)

    def _need_entropy(self, meta_info=None) -> bool:
        try:
            entropy_coeff = float(self.config.get('entropy_coeff', 0.0))
        except (TypeError, ValueError):
            entropy_coeff = 0.0
        return entropy_coeff != 0.0

    def _forward_micro_batch(self, micro_batch, temperature,
                             compute_entropy=True) -> Tuple[Optional[torch.Tensor], torch.Tensor]:
        """
        Returns: 
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        compute_entropy = self._need_entropy(data.meta_info)
        log_probs_lst = []
        entropy_lst = [] if compute_entropy else None
        ...
        entropy = torch.concat(entropy_lst, dim=0) if compute_entropy else None
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for data in micro_batches:
                ...
                if self.config.get('clip_high', None) is not None:
                    # Optional upper clip for very large probability ratios.
                ...
                # Use the k3 KL estimate to reduce variance.
