...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        # The actor is loaded in fp32 so the optimizer state is not initialized in bf16.
        ...
        with init_context(), warnings.catch_warnings():
            ...
            attn_implementation = self.config.model.get(
                'attn_implementation',
                os.environ.get('VERL_ATTENTION_IMPLEMENTATION', 'flash_attention_2'),
            )
            if not self._is_ref:
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name != 'vllm':
            raise NotImplementedError("Public RUAR training supports actor_rollout_ref.rollout.name=vllm.")
