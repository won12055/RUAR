...
def _ulysses_flash_attention_forward(
    ...
    attn_output = _flash_attention_forward(query_states, key_states, value_states, *args, position_ids=position_ids,
                                           **kwargs)
...
def patch_vlm_for_ulysses_input_slicing(model_class: type):
    ...
    def _create_ulysses_wrapped_decoder_forward(original_forward):
        def ulysses_wrapped_decoder_forward(self, *args, **kwargs):
            ...
            slice_now = inputs_embeds is not None and current_ulysses_sp_size > 1 and getattr(self,
                                                                                              "_needs_initial_slice",
                                                                                              True)
            if slice_now:
